...
from django.views.generic import TemplateView, View #About-page, #Contact
...
from django.http import JsonResponse #Subcribe
...
class HomeView(ListView):
    ...
    #feature post trending left  
        context ["featured_post"] = (
             Post.objects.filter(published_at__isnull=False, status="active") 
             .order_by("-published_at", "-views_count").first()
             )
        context ["featured_posts"] = Post.objects.filter(
            published_at__isnull=False, status="active").order_by("-published_at", "-views_count") [1:4]
...
class PostDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["previous_post"] = Post.objects.filter(published_at__isnull=False, status="active", id__lt=obj.id).order_by("-id").first()
        #8, 9, 10
        context ["next_post"] = (
            Post.objects.filter(
